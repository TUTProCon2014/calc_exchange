...
std::vector<std::string>  calc_exchange(std::vector<std::vector<ImageID>> const & target_, int select_costrate, int exchange_costrate, size_t max_select_times){
    std::vector<std::vector<Index2D>> target(target_.size());
    for (size_t i = 0; i < target_.size(); ++i){
        for (size_t j = 0; j < target_[i].size(); ++j)
            target[i].emplace_back(target_[i][j].get_index());
    }
